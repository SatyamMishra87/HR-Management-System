...
const SalaryCalculationfunction = async (userId, startDate, endDate, grossSalary) => {
    ...
    for (const dayInfo of dayInfos) {
        ...
        // WEEKEND/HOLIDAY OVERTIME HANDLING
        ...
        for (let lp of leaveApplication) {
            ...
            if (leaveDay) {
                ...
                console.log(`Leave found: ${leaveFraction} day, ${isPaidLeave ? 'PAID' : isUnpaidLeave ? 'UNPAID' : 'ABSENT'}`);
                break;
            }
        }
        ...
        if (attendanceForDay && attendanceForDay.clockIn && !attendanceForDay.clockOut) {
            ...
            if (!penalisationPolicy?.enableMissingClockOutPenalty) {
                absentDays += 1;
                console.log(" No missing clock out policy - 1 day absent");
                continue;
            }
            ...
            if (penalisationPolicy.missingClockOutPenaltyType === ENUMS.missingClockOutPenaltyType.AMOUNT) {
                ...
                presentDays += 1;
                console.log(` Amount penalty: ₹${penalisationPolicy.missingClockOutPenaltyAmount} - 1 day present`);
        ...
        if (leaveFraction === 1) {
            ...
            console.log("Full day leave - no attendance processing");
            continue;
        }

        if (leaveFraction === 0.5 && !attendanceForDay) {
            if (isPaidLeave) paidLeaveDays += 0.5;
            else if (isUnpaidLeave) unpaidLeaveDays += 0.5;
            absentDays += 0.5;
            console.log("Half day leave + no attendance - 0.5 absent");
            continue;
        }

        if (!attendanceForDay && leaveFraction === 0) {
            absentDays += 1;
        ...
        if (hasLateIn && hasEarlyOut) {
            console.log(` Both Late In & Early Out detected - Rule: ${penaltyRule}`);
            switch (penaltyRule) {
                case 1:
                case 2:
        ...
        // Track what remains after penalties
        ...
        if (applyLateInPenalty) {
            ...
            if (penalisationPolicy.applyInstantPenaltyForLateIn) {
                console.log("applyInstantPenaltyForLateIn ")
                if (penalisationPolicy.instantLateInPenaltyType === ENUMS.instantPenaltyType.AMOUNT) {
                    ...
                    if (dayType === ENUMS.leaveType.FULL) {
                        ...
                        console.log(` Instant FULL DAY ${isPaid ? 'PAID' : 'UNPAID'} leave: ${remainingAttendance} day`);
                        remainingAttendance = 0;
                    } else {
                        const halfDayPenalty = Math.min(0.5, remainingAttendance);
                        console.log("remainingAttendance under applyinstantpenlaitylatein", remainingAttendance);
                        if (isPaid) paidLeaveDays += halfDayPenalty;
                        else unpaidLeaveDays += halfDayPenalty;
                        remainingAttendance -= halfDayPenalty;
                        console.log(` Instant HALF DAY ${isPaid ? 'PAID' : 'UNPAID'} leave: ${halfDayPenalty} day (remaining: ${remainingAttendance})`);
                    }
                }
            } else {
                totalLateCount++;
    ...
    console.log("COUNTER-BASED PENALTY PROCESSING");
    ...
    console.log("\n OVERTIME:");
    console.log(`Total overtime amount: ${totalOvertimeAmount.toFixed(2)}`);
